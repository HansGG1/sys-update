...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(125, 65, 210, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
    ...
    {
        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ImColor(13, 12, 17), 5);

        if (it_anim->second.a < size.x - 2)
        {
            ImGui::GetWindowDrawList()->AddCircleFilled(ImVec2(clickpos.x, clickpos.y), it_anim->second.a, ImColor(18, 15, 26, 255));
        }
        else
        {
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ...
    it_anim->second.a = ImLerp(it_anim->second.a, Enabled ? ImColor(125, 65, 210) : ImColor(125, 65, 210, 0), g.IO.DeltaTime * 8.f);
    it_anim->second.b = ImLerp(it_anim->second.b, (hovered && !Enabled) ? ImColor(125, 65, 210, 22) : ImColor(125, 65, 210, 0), g.IO.DeltaTime * 8.f);

    // Hover tint
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(it_anim->second.b), 5);

    // Active: faint fill + left indicator bar + glow
    {
        float alpha = it_anim->second.a.w;  // 0 = inactive, 1 = active
        if (alpha > 0.01f)
        {
            window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, ImColor(125, 65, 210, (int)(alpha * 20)), 5);
            window->DrawList->AddRectFilled(
                total_bb.Min + ImVec2(0.f, 5.f),
                ImVec2(total_bb.Min.x + 3.f, total_bb.Max.y - 5.f),
                ImColor(125, 65, 210, (int)(alpha * 255)), 2.f);
            window->DrawList->AddRectFilled(
                total_bb.Min + ImVec2(0.f, 5.f),
                ImVec2(total_bb.Min.x + 12.f, total_bb.Max.y - 5.f),
                ImColor(125, 65, 210, (int)(alpha * 28)), 2.f);
        }
    }

    // Icon at left, label text to its right
    {
        PushFont(FrameWork::Assets::FontAwesomeSolid);
        float iconAlpha = 0.25f + it_anim->second.a.w * 0.75f;
        ImVec2 iconSz = CalcTextSize(Icon);
        window->DrawList->AddText(
            total_bb.Min + ImVec2(14.f, total_bb.GetHeight() * 0.5f - iconSz.y * 0.5f),
            ImColor(1.f, 1.f, 1.f, iconAlpha), Icon);
        PopFont();
    }

    // Label text
    {
        float textAlpha = 0.22f + it_anim->second.a.w * 0.78f;
        window->DrawList->AddText(FrameWork::Assets::InterSemiBold, 11.f,
            total_bb.Min + ImVec2(34.f, total_bb.GetHeight() * 0.5f - 5.5f),
            ImColor(1.f, 1.f, 1.f, textAlpha), Label);
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.color = ImLerp(it_anim->second.color, *v ? ImColor(125, 65, 210) : ImColor(13, 12, 17), g.IO.DeltaTime * 10.f);
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(125, 65, 210, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
...
bool ImGui::KeyBind(const char* label, int* k)
{
    ...
    ImGui::PushFont(FrameWork::Assets::InterRegular14);
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(13, 12, 17), 5);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items_count)
{
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(13, 12, 17), 4);
    ...
    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13.f), 3.f + it_anim->second.active_circle_anim, ImColor(125, 65, 210));
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(13, 12, 17, 255)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(13, 12, 17), 5);
    ...
    if (value > 1)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 1), frame_bb.Min + ImVec2(it_anim->second.a, grab_bb.Max.y - grab_bb.Min.y + 3), ImColor(125, 65, 210), rounding);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, hovered ? ImColor(125, 65, 210, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    else
    {
        ...
        it_anim->second.a = ImLerp(it_anim->second.a, hovered ? ImColor(125, 65, 210, 50).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
